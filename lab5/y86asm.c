...

{
    ...
    symbol_t *pointer = symtab -> next;

    while (pointer != NULL)

    {

        if (strncmp(pointer->name, name, strlen(name)) == 0)
...

{
    ...
    while ((IS_DIGIT(cur + length)) || (IS_LETTER(cur + length))) 
    ...
    char *new_name = (char *)malloc(length + 1);

    strncpy(new_name, cur, length+1);
      ...
      case I_DIRECTIVE: {
		...
		y86bin -> codes[4] = 0xFF;
        	...
        	y86bin -> codes[0] = value & 0xFF;
       	 	...
       	 	y86bin -> codes[1] = (value & 0xFF00) >> 8;

    		y86bin -> codes[2] = (value & 0xFF0000) >>16;

        	y86bin -> codes[3] = (value & 0xFF000000) >>24;
...
int relocate(void)
{
	...
	symbol_t *new_sym = find_symbol(tmp -> name);
	...
	if (new_sym == NULL) {
        ...
        /* relocate y86bin according ityIpe */

	else {

	  tmp->y86bin->codes[tmp->y86bin->bytes-4] = (new_sym->addr)&0xFF;

          tmp->y86bin->codes[tmp->y86bin->bytes-3] = (new_sym->addr>>8)&0xFF;

          tmp->y86bin->codes[tmp->y86bin->bytes-2] = (new_sym->addr>>16)&0xFF;

          tmp->y86bin->codes[tmp->y86bin->bytes-1] = (new_sym->addr>>24)&0xFF;
...
int binfile(FILE *out)
{
    ...
    while (line) {
        if (line -> y86bin.bytes) {
            align = line -> y86bin.addr - address;
            while (align > 0) {
		align--; 
                fprintf(out, "%c", 0);
	    }

            judge=fwrite(line->y86bin.codes, sizeof(byte_t), line->y86bin.bytes, out);
	    if (judge != line -> y86bin.bytes)
		return -1;
            address = line -> y86bin.addr + line -> y86bin.bytes;
        }
        line = line -> next;
    }
