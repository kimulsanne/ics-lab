...
int bitMask(int highbit, int lowbit) {
  return ((0|((1<<highbit<<1)+~0x0))&(~((1<<lowbit)+~0x0)));
}
/* 
...
int isLess(int x, int y) {
  int sx=((x>>31)&1);
  int sy=((y>>31)&1);
  return !(((~((x+~y+1)>>31)&1)&~(sx^sy))+(((sx^sy)&(sx^1)&(sy&1))));
}
/* 
...
int reverseBytes(int x) {
  return ((x>>24&0xff)+((x<<8>>24&0xff)<<8)+((x<<16>>24&0xff)<<16)+(x<<24));
}
/* 
