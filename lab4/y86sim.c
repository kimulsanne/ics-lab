...
stat_t nexti(y86sim_t *sim)
{
	...
	{
	  byte_t codefunb = 0;
	  if (!get_byte_val(sim->m,next_pc, &codefunb)) {
		  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
		  return STAT_ADR;
	  }
	  regid_t rA = GET_REGA(codefunb);
	  regid_t rB = GET_REGB(codefunb);
	  next_pc++;
      ...
      case I_IRMOVL: /* 3:0 F:regB imm */
	{
	  ...
	  next_pc++;
	  long_t codefunl = 0;
	  if (!get_long_val(sim->m,next_pc, &codefunl)) {
		  err_print("PC = 0x%x, Invalid instruction address", sim->pc);
		  return STAT_ADR;
	  }
	  next_pc+=4;
	  set_reg_val(sim->r,rB,codefunl);
	  sim->pc = next_pc;
	  break;
	}
	  ...
	  set_long_val(sim->m, valB + codefunl, valA);
	  sim->pc = next_pc;
	  break;
        }
      ...
      case I_MRMOVL: /* 5:0 regB:regA imm */
        {
          ...
          long_t valB = get_reg_val(sim->r, rB);
	  long_t codefunl2 = 0;
          if (!get_long_val(sim->m, valB + codefunl, &codefunl2)) {
                  err_print("PC = 0x%x, Invalid data address 0x%x", sim->pc, valB +codefunl);
                  return STAT_ADR;
          }   
          set_reg_val(sim->r, rA, codefunl2);
          sim->pc = next_pc;
          break;
        }
	...
	{
	  long_t codefunl = 0;
	  long_t valA = get_reg_val(sim->r, 4);
	  long_t valE = valA + 4;
	  set_reg_val(sim->r, 4, valE);
	  if  (!get_long_val(sim->m, valA, &codefunl)) {
	  ...
	  set_reg_val(sim->r, 4, valE);
          long_t codefunl = 0;
	  if (!get_long_val(sim->m, valE, &codefunl)) {
                  err_print("PC = 0x%x, Invalid stack address 0x%x", sim->pc, valE);
                  return STAT_ADR;
          }
